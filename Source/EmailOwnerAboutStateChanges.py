...
def email_owner_about_state_change(date_flag):
    ...
    for user in owners:
        ...
        user_state_changes.sort(key=lambda x: x['type'])
        msg = ['The state of the following samples owned by you has been changed by someone else:']

        html = dict_to_html(user_state_changes, 
                            ['date', 'sample_id', 'sample_type', 'from_state', 'to_state', 'user_name', 'vial_location', 'comments'],  
                            ['Date', 'Sample Id', 'Sample Type', 'From State', 'To State', 'Changed by', 'Location', 'Comments'])
        msg.append(html)
